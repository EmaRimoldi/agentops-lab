...
def main(argv=None) -> None:
    ...
    if args.command == "parallel":
        from agentops_lab.modes.parallel import main_parallel
        main_parallel(rest)
    elif args.command == "parallel-shared":
        from agentops_lab.launcher import main_parallel_shared
        main_parallel_shared(rest)
    elif args.command == "single-long":
        from agentops_lab.modes.single_long import main_single_long
        main_single_long(rest)
    elif args.command == "single-memory":
        from agentops_lab.launcher import main_single_memory
        main_single_memory(rest)
    elif args.command == "swarm":
        from agentops_lab.modes.swarm import main_swarm
        main_swarm(rest)
